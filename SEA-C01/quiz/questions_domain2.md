...
## Q116 (Single Answer)
A Security Engineer needs to verify in near-real-time which masking policies are currently applied to columns in a specific Snowflake database, with minimal latency. Which view provides this information?

- A) SNOWFLAKE.ACCOUNT_USAGE.POLICY_REFERENCES (up to 3-hour latency, account-wide)
- B) INFORMATION_SCHEMA.POLICY_REFERENCES (near-real-time, current database scope)
- C) SNOWFLAKE.ACCOUNT_USAGE.MASKING_POLICY_HISTORY
