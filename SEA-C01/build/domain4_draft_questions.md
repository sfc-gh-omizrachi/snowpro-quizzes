...
## Q22 (Scenario)
A Security Engineer is tasked with implementing near-real-time detection for unauthorized privilege escalation. Which Snowflake feature is BEST suited for alerting when a user is granted ACCOUNTADMIN?

- A) A Snowflake Alert on SNOWFLAKE.ACCOUNT_USAGE.GRANTS_TO_ROLES where ROLE = 'ACCOUNTADMIN'
- B) A row-access policy that blocks GRANT commands for ACCOUNTADMIN
- C) Trust Center's Security Essentials scanner
- D) An MFA policy that blocks ACCOUNTADMIN usage
